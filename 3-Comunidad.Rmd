...
La Pampa argentina (Fig. 3.1) es una amplia llanura con una extensión de más de 450 mil km^2^, de las
...
La Pampa Ondulada es la zona más fértil y productiva de la región Pampeana, donde más del
...
![La Pampa Argentina y sus principales divisiones [@morello1997]](Figures/chap3_morello1997.png)
...
fauna edáfica pertenecientes a suelos de los partidos de Chivilcoy y Navarro [@velazco2023a],
...
|----------|------------|------------|
| Reserva (R)  | 34°35'34"S | 59°04'16"W |
|          | 34°35'35"S | 59°04'18"W |
|          | 34°35'36"S | 59°04'20"W |
| Pastizal (P) | 34°35'38"S | 59°04'22"W |
|          | 34°35'38"S | 59°04'23"W |
|          | 34°35'38"S | 59°04'25"W |
| Agrícola (A)| 34°35'22"S | 59°04'05"W |
...
(ancho corporal menor a 200 μm), la **mesofauna** en el que se incluyen los
...
variables químicas que caracterizan estos ambientes edáficos.
...
pastoreos, y; c) un área clausurada por más de 30 años considerada como Reserva (R) cuya
...
proceso que llevo aproximadamente una semana. <!--anexo tal vez:(Fig 3.x)[Fig 3.x colocar mosaico de imágenes donde estamos suelo lupa y balanza]-->
...
Crassiclitellata) fueron identificadas hasta especies [@reynolds1996; @satchell1983; @demichis1999] 
y la macrofauna restante fué separada en diferentes rangos
...
apilados de 4 mm, 2mm, 1mm, 500 μm, 250 μm y 170 μm; la mesofauna se
colecta de lo que queda atrapado en los tamices desde los 1000 μm hasta las 170 μm.
...
entre las comunidades pertenecientes a diferentes sistemas de uso del suelo, por lo que el valor p de 
probabilidad indica que las comunidades son diferentes entre sí considerando el p~valor~ < 0,1, 
el valor del estadístico R nos indica el grado de separación que existe entre las comunidades
comparadas entendiendo que si este se aproxima al valor 1 el grado e separación es más marcado;
a la vez para conocer cuáles son las comunidades similares entre sí, 
se calculó el dendograma basado en el agrupamiento jerárquico con el método de enlace promedio. 
Para ambos análisis se utilizó el coeficiente de distancia de Bray-Curtis [@legendre2012;@koleff2003].
...
En los suelos de Luján la matriz de correlaciones de a pares (Tabla 3.2) nos muestran que las
variables están poco correlacionadas entre si con excepción del nitrógeno que se
correlaciona negativamente con la respiración heterotrófica ($\rho$: p~valor~ < 0,1). 

El fósforo presenta una situación marginal, con su p-valor igual a $\rho$: p~valor~ = 0,1, con 
relación a la respiración heterotrófica, estas variables se correlacionan entre sí negativamente.
También hay una correlación positiva entre el fośforo y nitrógeno $\rho$ = 0,57 pero su 
significancia esta un punto por arriba de la situación marginal $\rho$: p~valor~ = 0,11.
...
Las correlaciones entre pares de las variables (Tabla 3.2) medidas en los suelos Argiudoles de
...
Los valores del pH (Tabla 3.3), en los suelos argiudoles de Luján, no mostraron diferencias
...
lote lograrían "homogeneizar" el valor de este en la superficie agrícola.
...
de sistema de uso de suelo Pastizal sobre el lado negativo del eje. Con estas dimensiones (Fig. 3.2) 
pueden observarse que cada sistema de uso del suelo se separan claramente.
...
con una gran importancia (r= -0.85) para la relación de hongos:bacterias y una importancia media 
para el pH.
...
argiudoles de la reserva, también la respiración heterotrófica se relaciona cercanamente con la Reserva 
y levemente con los suelos del pastizal, esta variable tiene una fuerte correlación negativa con la 
...
En el biplot (Fig. 3.2) se observa una clara separación a lo largo de la primera dimensión 
del centroide del sistema del pastizal naturalizado sobre el lado positivo del eje de
coordenadas. 

Los sistemas de uso agrícola y mixto que se ubican hacia el lado opuesto y en posiciones 
similares sobre primer eje dimensional, se diferencian en su ubicación 
en relación al segundo eje de coordenadas ya que el centroide del sistema ganadero se ubica
hacia el lado positivo de la segunda dimensión
y el sistema de uso agrícola intensivo se ubica sobre el sentido negativo del segundo 
eje dimensional.
...
dimension. El fósforo asimilable se ubica en sentido opuesto al pH con una importante 
relación con el primer eje de coordenadas y relacionado con 
los centroides de los sistemas agrícola intensivo y mixto. El porcentaje de nitrógeno 
...
establecen relaciones con puntos muestrales en los diferentes sistemas de uso del suelo
no siendo específicos a aluguno de ellos.
...
En los suelos de la Luján el máximo valor de la riqueza (Tabla 3.4) en unidades taxonómicas se halló
en suelos del pastizal, en suelos de Chivilcoy y
...
En suelos de Luján, el sistema agrícola presenta la menor riqueza de lombrices.
...
El índice de diversidad de Shanon-Wiener (Tabla 3.5) no mostró diferencias significativas 
...
unidades taxonómicas en cada sistema de uso del suelo y con abundancias bajas de estos taxones. 
...
Los dendrogramas (Fig 3.3) muestran la similitud de las comunidades
...
situación de proximidad entre sitios del Pastizal y la Reserva; **b)** en los suelos de Chivilcoy
...
En ambas localidades, a lo largo del primer eje se resume la diversidad de taxones que se hallan
en las comunidades edáficas por lo tanto se observa que este eje relaciona la
abundancia de los taxones ubicándo más hacia la izquierda los taxones raros como aquellos
de la macrofauna y más hacia la derecha los taxones abundantes que corresponde principalmente
a la microfauna.
...
En los suelos de Luján (Fig 3.4) la separación del sistema agrícola de los demás sistemas de uso del
suelo es clara dirigiéndose hacia el lado negativo del segundo eje de coordenadas. El
sistema de la reserva se posiciona en el nivel superior del segundo eje y el sistema
del pastizal se muestra entre ambos. Por lo que podría interpretarse una
situación de gradiente con respecto a la composición de las comunidades del suelo.

Al observar los principales grupos taxonómicos, se forma un agrupamiento entre los
nemátodos, los enquitreidos y los astigmatas en el extremo positivo de la primera dimensión
de las coordenadas principales. En el extremo opuesto, se agrupan los taxones que
corresponden principalmente a la macrofauna, de mayor a menor serían las lombrices de
tierra, herbívoros, detritívoros, predadores y fungívoros. 

Entre los dos grupos conformados en los extremos del primer eje de coordenadas,
descritos anteriormente, se dan situaciones particulares, 
...
una situación cercana al centro de coordenadas (0,0) lo que indica que su variabilidad no se
encuentra bien representada por estas dimensiones. Los ácaros
oribátidos muestran con una leve relación con los vectores de los sitios que corresponden al suelo
de la Reserva, en esta situación también se hallan los macroartrópodos omnívoros con una mayor
relación con el suelo de la Reserva, los tardígrados se hallan
fuertemente relacionados con el sistema de la Reserva indicando la presencia de retención y saturación
de humedad en el suelo. Los colémbolos se relacionan con los argiudoles del sistema del pastizal.
...
(Mesostigmatas) sin una relación aparente con algún sistema de uso del suelo. 
...
Las caracterizaciones químicas y biológicas de los suelos se correlacionan entre sí 
...
separación uno del otro. Existen claras relaciones con las prácticas de manejo, relacionadas al uso 
agrícola, que aporta ciertos macronutrientes como el fósforo y el nitrógeno mediante el uso de
...
al aumento de nutrientes en estos suelos, disminuyendo su actividad y potencialmente reduciendo los 
procesos de descomposición y mineralización de la materia orgánica.
...
La diversidad alfa puntual de las comunidades edáficas en los suelos Argiudoles típicos se mantiene 
similar a pesar de los efectos que la actividad antrópica pudiera tener sobre el ambiente edáfico, 
sin embargo,
...
de que la fauna del suelo posee una gran redundancia de taxones para diferentes funciones ecológicas 
claves[@anderson1978], tales como, el control de poblaciones de la microflora, la fragmentación de la 
hojarasca, la herbivoría, la formación de canales a través del suelo, etc. 

Si se reflejan en cambios en la diversidad Beta, es decir, cambios entre los actores 
y sus abundancias que 
...
inclusive a grandes distancias y a frecuencia de muestreo y las variaciones que existen en estas 
características se relacionan con el tiempo de uso y con la forma de uso que se realiza sobre el suelo.
