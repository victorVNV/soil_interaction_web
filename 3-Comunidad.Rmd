...
Riqueza<-specnumber(D[,-120],groups = D$sitio) #Riqueza[["R"]]
Riqueza_nematodos<-specnumber(D[40:44],groups = D$sitio)
Riqueza_lombrices<-specnumber(D[c(2:4,19,61,87:89,93,98,99)],groups = D$sitio)

Riqueza_acari<-specnumber(D[c(5,9,18,26:31,39,45,47:53,55:59,81,82,64:66,68,76,114:117,
                            105,106,108:111,85,102,103,65:67)],groups = D$sitio)
Riqueza_sarcoptiformes<-specnumber(D[c(5,29:31,47:50,57,64:65,68,76,116:117,103,
                            105,106,108,109,85,95)],groups = D$sitio)

Riqueza_mesostigmata<-specnumber(D[c(18,26,28,39,45,51,56,68,66,115,102,96,97)],groups = D$sitio)
Riqueza_prostigmata<-specnumber(D[c(9,27,30,81,82,52,53,55,58,59,76,114,110)],groups = D$sitio)
Riqueza_collembola<-specnumber(D[c(12,16,32,67,46)],groups = D$sitio)
Riqueza_otra_mesofauna<-specnumber(D[c(11,23,104,79)],groups = D$sitio)
Riqueza_macrofauna<-specnumber(D[c(1,6:8,10,13:15,17,20:25,33:38,54,60,62,63,69,
                                 70:75,78,80:84,86,90:92,94,100,101)],groups = D$sitio)
Sitios=levels(as.factor(D$sitio))
Sitios <- factor(Sitios, levels = c("R", "P", "A"))
S_UNLu<-data.frame(Sitios=c("R", "P", "A"),S=c(Riqueza[["R"]],Riqueza[["P"]],Riqueza[["A"]]),
                   S_lombrices=c(Riqueza_lombrices[["R"]],
                         Riqueza_lombrices[["P"]],Riqueza_lombrices[["A"]]),
                   S_collembola=c(Riqueza_collembola[["R"]],
                                  Riqueza_collembola[["P"]],Riqueza_collembola[["A"]]),
           S_sarcoptiformes=c(Riqueza_sarcoptiformes[["R"]],
                              Riqueza_sarcoptiformes[["P"]],Riqueza_sarcoptiformes[["A"]]),
           S_mesostigmata=c(Riqueza_mesostigmata[["R"]],
                            Riqueza_mesostigmata[["P"]],Riqueza_mesostigmata[["A"]]),
           S_prostigmata=c(Riqueza_prostigmata[["R"]],
                           Riqueza_prostigmata[["P"]],Riqueza_prostigmata[["A"]]),
           S_otra_mesofauna=c(Riqueza_otra_mesofauna[["R"]],
                              Riqueza_otra_mesofauna[["P"]],Riqueza_otra_mesofauna[["A"]]),
           S_macrofauna=c(Riqueza_macrofauna[["R"]],
                          Riqueza_macrofauna[["P"]],Riqueza_macrofauna[["A"]]),
           row.names = NULL)
...
en suelos de la reserva ($S_{reserva=86$). En suelos de Chivilcoy y
Navarro la máxima riqueza de unidades taxonómicas es similar en suelos agrícolas intensivo y mixto.

La riqueza de colémbolos en los suelos argiudoles en general fue similar encontrándose 5 
taxones diferentes en los diferentes sistemas de uso del suelo.
...
los mayores valores se observaron en suelos de Luján con una riqueza máxima de 21 en el
sistema de la reserva. En los suelos de Chivilcoy y
Navarro la riqueza máxima se observó tanto en el sistema mixto como en el sistema agrícolac 
intensivo con un valor de 9.

Para la riqueza en unidades taxonómicas de ácaros mesostigmatas (Acari: Mesostigmata) 
se observa el mayor valor en el sistema de la Reserva seguidos por el sistema agrícola en suelos de Luján, en Chivilcoy y Navarro se hallaron hasta 5 taxones diferentes en los sistemas con uso de suelo antropizado.
...
colnames(H)<-c("","Total","  ","Lombrices","   ","Mesofauna","    ","Macrofauna","     ","Microfauna","         ")

```

**Tabla 3.x Índice de Diversidad de Shannon (H'). Total: refiere a la diversidad de toda la fauna edáfica; Lombrices, Mesofauna, Macrofauna, Microfauna, refiere a la diversidad calculaa para grupos de la fauna edáfica clasificados según su rango de tamaños. X̅ ± σ; expresa el valor promedio más el desvío estandar de la diversidad del componente, X̂ expresa el valor de la mediana de la diversidad del componente**
...
taxones en diferentes clases de tamaño 
...
El análisis de similitudes (ANOSIM) nos indica que existen disimilaridades estadísticamente
significativas ($R: p_{valor}<0.05$) entre las comunidades que pertenecen a diferentes sistemas 
de uso del suelo, tanto en los suelos de Luján como en los suelos de Chivilcoy y Navarro.
...
los valores del estadístico que indican la proporción de disimilitud existente entre los grupos,
es decir, la separación de los sistemas de uso del suelo en cada zona de estudio. 
En los suelos de Luján las comunidades de la fauna edáfica muestran importantes separaciones 
entre los sistemas de uso el suelo, por su parte, las comunidades de microartrópodos y lombrices de
suelos de Chivilcoy y Navarro también se separan entre los diferentes sistemas de uso del suelo aunque 
con menos fuerza.
...
taxonómica del grupo de la macrofauna puede ser más heterogéneo debido a diferentes actores en cada 
sistema de uso del suelo y con abundancias bajas de taxones. 
...
**Figura 3.x Dendrogramas de disimilitud**
...
(`r r_RO_anosim`), un cierto grado de similitud existe entre dos de los sitios del sistemas
de uso del pastizal naturalizado, por otra parte, en grado de similitud el sistema agrícola
...
Las primeras dos dimensiones del análisis de coordenadas principales (Fig. 3.x) 
...
edáficas para los suelos de Luján y los suelos de Chivilcoy y Navarro, respectivamente.
