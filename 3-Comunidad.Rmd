...
La conductividad eléctrica medida en milisiemens por
centímetro (milisiemens por centímetro $\frac{mS}{cm}$), para los suelos argiudoles de
Luján, fue determinada para el estrato de 10 a 20 cm. En cada sitio se observan claras
diferencias y esto se convalida con el test de K-W p:H\<0.01, en el gráfico de cajas y
bigotes se observa el mayor valor de CE se da en el pastizal natural.
...
significativas estadísticamente para la prueba de Kruskal-Wallis (H: p<0.01).
El fósforo al ser un nutriente poco movil y poco disponible en el suelo puede asociarse 
su amplitud de valores para el uso agrícola en ambos estratos
con el efecto de la fertilización de este nutriente sobre el suelo. 
En los demás sitios los valores 
de la concentración de fósforo fueron similares en ambos estratos tanto para la reserva como
para el pastizal natural.
...
En los suelos argiudoles de Chivilcoy y Navarro, la variación del fósforo entre los sistemas de uso del suelo no mostraron diferencias significativas (K-W H:p>0.05), se observan en los
gráficos de cajas que la mediane del sistema G fué levemente mayor a las medianas del 
pastizal naturalizado y el sistema agrícola cuyos valores de mediana fueron mróximos entre 
sí. Además en el sistema intensivo agrícola los valores en partes por millón del fósforo 
asimilable muestran una gran amplitud, que podría tratarse del efecto del balance adición de nutriente / consumo del cultivo, en cambio, en el sistema del pastizal naturalizado 
el rango de variabilidad es más estrecho reflejando las condiciones ambientales más homogénea del sistema.
...
significaivas en las medianas entre los sitios (K-W H:p<0.01),
la reserva muestra el mayor valor de resiracion seguido de cerca por el pastizal y 
con el cual no muestra diferencias significativas, pero si se puede observar que las cajas 
no se superponen entre sí. El valor mas bajo de la actividad respiratoria se da en el 
sistema agrícola el cual difiere marcadamente de ambos grupos. 

Para estos suelos, podríamos decir que en los suelos de lotes que se encuentran
permanentemente cubiertos 
la actividad respiratoria biológica de la microbiota es mayor.
...
En los suelos argiudoles de Chivilcoy y Navarro, la respiración heterotrófica en 
el sistema agrícola fue menor y se diferenció significativamente (K-W H:p<0.05) de los 
...
respiración para el sistema del pastizal naturalizado pero con valores mínimos de actividad 
respiratoria por arriba de los valores mínimos de los demás sistemas de uso.
...
Pero observando las medianas, en la reserva la
relación de eucariotas:procariotas es mayor, lo que indica una mayor proporcion de la
actividad de hongos en este sitio y ello está ligado a un sistema más maduro en términos de 
sucesión ecológica y a los canales de energía en sistemas detitívoros del suelo 
(.@.cita/cita) o a la
actividad de descomposición de la hojarasca por parte de hongos y micorrizas. 
En el caso del lote agrícola el valor de su mediana es similar al del pastizal pero el rango
...
valor del pastizal refleja una mayor homogeneidad en las proporciones de taxones 
del microbioma en estos suelos.
...
suelo de la Reserva tiene un mayor biomasa que en los demás sistemas, además se observa el valor
más bajo de la mediana y una dispersión más estrecha en el sistema de la Agrícola y en el 
pastizal un valor de la mediana del peso similar al de la Reserva pero con muy poca variación.
...
print(round(as.matrix(corr_fqb_unlu$P),2))
print(round(as.matrix(corr_fqb_unlu$r),2))
...
correlaciona negativamente con la respiración heterotrófica, 
esto se observa mediante el valor de significacia del p-valor de Spearman $\rho$: p<0.05, 
se observa tambien una significancia marginal con valor $\rho$: p\<0.1 entre la 
conductividad eléctrica y la relación hongos/bacterias con correlaciones negativas 
por lo que indicaría que suelos más salobres disminiyen la proporción de hongos por gramo de
suelo.
El P también presenta una situación marginal, con su p-valor igual a $\rho$: p\=0.1,
con relación a la respiración heterotrófica, estas variables se corelacionan entre sí negativamente.
También hay una correlación positiva entre el fośforo y nitrógeno $\rho = 0.57$ pero su significancia
esta un punto por arriba de la situación marginal  $\rho$: p=0.11.
...
La corelacion entre el nitrógeno y el fósforo es positiva, aunque no significativas, 
lo que puede ser debido al efecto de la fertilización con compuestos que poseen ambos
componentes.

```{r cor_RO,echo=FALSE}
corrRO<-Hmisc::rcorr(as.matrix(RO_db_full[,c(10:14,19)]),type = "spearman")

print(round(as.matrix(corrRO$P),2))
print(round(as.matrix(corrRO$r),2))

#corrplot::corrplot(cor(as.matrix(RO_db_full[,c(10:14,19)])),tl.pos="t",type = "upper",tl.col = "black",tl.cex = .8,bg="brown4",diag=FALSE)
```

Las correlaciones entre pares de las variables medidas en los suelos Argiudoles 
de Chivilcoy y Navarro mostraron relaciones diferentes a aquellas de los suelos de Luján. 

El pH se correlacionó significativamente con el nitrógeno, el fósforo y la respiración heterotrófica, las correlaciones fueron negativas entre el pH y el nitrógeno y el fósforo, por lo que aumentos en la cantidad de estos nutientes disminuyen el valor del pH
y positiva entre el pH y la respiración heterotrófica, por lo que la actividad biológica 
aumenta los niveles de pH volviendo el medio más alcalino.

La conductividad eléctrica se correlacionó significativamente con la materia orgánica,
el fósforo y la respiración heterotrófica. Con relación a la materia orgánica la 
correlación es negativa, esto puede asociarse al efecto de las sales de sodio
de dispersar la materia orgánica del suelo, y, correlaciones positivas se 
dan con el fósforo y levemente con la respiración heterotrófica.
...
**Análisis de Componentes Principales (PCA) de las variables físicas-químicas en suelos Argiudoles**
...
tres componentes principales tienen autovectores mayores a uno. A la vez los ejes acumulan el 
80% de la varianza según el criterio de ordenación de los componentes principales.
...
Las primeras 2 dimensiones de PCA resumen el 64,77 % de la variabilidad entre las variables
físico químicas y los sitios. Con estas dimensiones pueden observarse, en el biplot,
que cada sistema de uso del suelo se separan claramente.

Sobre la dimensión 1 los centroides del sistema de uso del suelo Agrícola y el sistema
de la Reserva se separan en cuadrantes opuestos. A la vez el porcentáje de nitrógeno del suelo 
muestra una correlación importante (r = 0.77) con el lado positivo del primer componente principal 
y esta variable se relacionó más con el sistema Agrícola. 
Sobre la dimensión 1 todas las variables presentan correlaciones importantes con excepción de la conductividad eléctrica. 

Sobre la dimensión 2 se ubica el centroide de sistema de uso de suelo Pastizal sobre el 
lado negativo del eje. La variable conductividad eléctrica se dirige hacia el lado negativo 
del segundo componente a la vez que apunta al centroide del pastizal, estableciendo una 
clara relación entre estos.

El pH y la relación hongos:bacterias se encuentran cercanamente relacionadas a la vez que los 
vectores apuntan hacia el centroide del sistema de Reserva. 
...
dirección de los vectores que se ubican entre los centroide y las coordenadas de los sitios 
Reserva y Pastizal.

El vector que resume la relación de la cantidad de hojarasca en superficie muestra una relación 
con el fósforo y apunta al centroide del lote agrícola.
...
Para los suelos argiudoles de Chivilcoy y Navarro las primeras tres dimensiones de los componentes
principales tienen autovalores mayores a 1 resumiendo el 78,2% de la variabilidad.

En el biplot se observa una separación a lo largo de la 
dimensión 1 del centroide del sistema del pastizal naturalizado que se ubica 
hacia el lado positivo del eje de coordenadas de los sistemas de uso agrícola y mixto 
que se ubican haciea el lado opuesto, estos dos últimos, se encuentran cercanos y se separan 
levemente con el sistema de uso agrícola intensivo sobre el sentido negativo del segundo eje 
dimensional.

La conductividad eléctrica se expresa fuertemente relacionada con la dimensión 1 en sentido 
positivo a la vez que el vector se dirige hacia el centroide del pastizal naturalizado.

La concentración del fósforo asimilable se relaciona con el lado negativo de la dimensión 2 y
el vector se halla relacionado, aunque levemente, con el sistema de uso agrícola intensivo.

El porcentaje de nitrógeno y el porcentaje de materia orgánica en el suelo se hallan 
en el mismo cuadrante del plano de coordenadas aunque estos no se relacionan estrechamente.
El porcentaje de nitrógeno tiene una correlación importante con la dimensión 3 ($r = 0.662$)y 
con la dimensión 1 ($r= -0.427$) y su vector se dirige más cercanamente hacia el centroide 
del sistema mixto agrícola-ganadero.
En comparación, al vector del porcentaje de nitrógeno, la materia orgánica dibuja un vector 
más extenso, su correlación es mayor con la dimensión 2 ($r = 0.71$) y 
también es importante la correlación con la dimensión 1 ($r = -0.498$), no se puede relacionar
al porcentaje de materia orgánica con algún sistema de uso de suelo por lo que se encuentra en
una situación intermedia, aunque gráficamente pareciese relacionarce con el centroide 
del sistema mixto.

El vector del potencial hidrógeno tiene un recorrido importante y apunta hacia el centroide
del pastizal natural, esta variable tiene correlaciones importantes con la primera y la segunda 
dimensión de los componentes principales ($r_{PCA_1} = 0.768$ y $r_{PCA_2} = 0.454$, respectivamente). 

Por arriba se observa a la respiración heterotrófica con una relación importante con el 
segundo eje dimensional ($r=0.754$), ubicándose en el cuadrante donde se sitúa el
sistema del pastizal naturalizado, relacionado marginalmente con varios de los sitios de este 
...
En los suelos de la Luján el máximo valor de la riqueza en unidades taxonómicas se 
halló en suelos de la reserva ($S_{reserva_1}=63$). De manera similar en suelos de
Chivilcoy y Navarro se halló la máxima riqueza en unidades taxonómicas en suelos 
del pastizal naturalizado ($S_{pastizal\; naturalizado\,_1}=63$).
...
los mayores valores se observaron en suelos de Luján con una riqueza máxima de 15 en 
el sistema de la reserva y de 11 en el sistema del pastizal. En los suelos de 
Chivilcoy y Navarro la riqueza máxima se observó en el sistema mixto con un valor de 9,
aunque en los demás sitios se mantuvo constante con 8 unidades taxonómicas.

La riqueza en unidades taxonómicas de ácaros mesostigmatas (Acari: Mesostigmata)
no fué muy diferente entre los suelos argiudoles. Se observa el mayor valor en 
el sistema de la Reserva seguidos por el sistema agrícola en suelos de Luján.

Los ácaros prostigmatas (Acari: Prostigmata) tambíen tuvieron una mayor riqueza en 
suelos de Luján, el mayor valor se observó en el sistema de uso agrícola. En comparación,
los suelos de Chivilcoy y Navarro tuvieron valores de riquezas similares entre sistemas 
de uso pero en el uso agrícola la riqueza fué menor que la de los sistemas mixto y 
pastizal naturalizado.

Las lombrices de tierra (Oligochaeta: Crassiclitellata) expresan una mayor riqueza en
los suelos de Chivilcoy y Navarro, con el valor máximo en el sistema del pastizal 
naturalizado ($S_{pastizal\,naturalizado\,_1}=8$), seguidos en valores máximos 
por el valor de riqueza en el sistema mixto agrícola ganadero.

La macrofauna tuvo la mayor riqueza de unidades taxonómicas en los suelos del
sistema Agrícola, seguidos por la riqueza en el pastizal y por detrás la riqueza de la 
macrofauna en el sistema de la Reserva.

**Índice de Shannon-Wiener**

El índice de diversidad de Shanon-Wiener no mostró diferencias significativas
entre los sistemas de uso del suelo, ni en suelos de Luján ni en suelos de 
Chivilcoy y Navarro. ($\chi^2 : p_{_{valor}} < 0.05$)
...
h<-rbind(g_H1,RO_div1)

kruskal.test(diversidad ~ sitio, data = h)

pairwise.wilcox.test(h$diversidad, h$sitio,data=h,
                     p.adjust.method = "BH",exact=FALSE)
...
se dío en el sistema agrícola tanto en promedio como en la mediana y con una 
baja dispersión. En suelos de Chivilcoy y Navarro el patrón se repite, en suelos donde
la actividad agrícola está presente, la diversidad es mayor tanto en promedio como en la
mediana.
...
entre sistemas de uso de suelos en argiudoles de Luján. Esto podría relacionarse
con la clasificación de los nemátodos según su aparato bucal en 5 categorías tróficas
fitófagos, fungívoros, baterivoros, redadores y omnívoros y tal véz podrían existir
diferencias en la diversidad alfa si se consideraran jerarquías taxonómicas de familias,
géneros o especies. Teniendo esto en consideración, puede observarse que el valor de 
diversidad fué menor en el sistema de la Reserva.
...
print(meso_H_UNLu %>% arrange(Habitat=="R 10",Habitat=="P 10",Habitat=="A 10",Habitat=="R humus",Habitat=="P humus",Habitat=="A humus",Habitat=="R fragmentado",Habitat=="P fragmentado",Habitat=="A fragmentado",Habitat=="R hoja",Habitat=="P hoja",Habitat=="A hoja"))
```
...
Si bien no se encontraron diferencias en los valores de la diversidad del índice de
shannon según la prueba de Kruskall-Wallis al p-valor menor de 0.05 el análisis de
similitudes de las comunidades nos muestra que existen disimilaridades en la comunidades.
...
Trabajaré con la base de datos completa sin considerar las submuestras, ya que las sub
muestras deben ser agrupadas para evitar la autocorrelación

Es necesario llevar las abundancias de los animales a valores de densidad ya que las
lombrices fueron tomadas con monolitos (factor= 16) y los microartrópodos con sacabocados
(factor= 509.2958).

A la vez se redondean los valores hacia el más bajo, de esta manera las lombrices tendrían
16 ind/m² y los microartrópodos 510 ind/m².

A partir de este se calcula la Biomasa de los organismos

```{r matricesRO1, include=FALSE, warning=FALSE}
#Anális de datos de ROSANA
#Carga de datos
...
```{r cor_RO1,echo=FALSE}
corrRO<-Hmisc::rcorr(as.matrix(RO_db_full[,c(10:14,19)]),type = "pearson");corrRO[1];corrRO[3]

#corrplot::corrplot(cor(as.matrix(RO_db_full[,c(10:14,19)])),tl.pos="t",type = "upper",tl.col = "black",tl.cex = .8,bg="brown4",diag=FALSE)
```

\*\* Análisis de Componentes Prinipales de los sitios de estudio en función de sus
variables físicas y químicas \*\*

```{r pcaFQ_RO1,echo=FALSE}
################
...
En PCA considerando las variables físicas y químicas de manera transformada y considerando
que las submuestras se encuentran agrupadas, se pueden observar la separación de 3 grupos
como una especie de función donde f y g de x se aproximan a h y por lo tanto sirven para
calcular valores en ella.

Por un lado inicia el sistema G, le sigue el A y el N al final.

Observando los vectores representan a G la concentración de nitrógeno en el suelo y un
tanto la proporción de fósforo casi marginalmente y tambien un poco relacionado con el A

Para A resulta clara la relacion con el contenido de MO y HR en el suelo, aunque también
la CE se relaciona con este y marginalmente con N.

En N varias variables se encontraron bien correlacionadas entre ellas el DAP el sodio el
pH y el Calcio, también la resistencia mecánica y los aumentos en sus valores son
asociados particularmente con el sistema N y el contenido de K que resulta importante.
...
```{r shanon_RO, include=FALSE, warning=FALSE}
# Diversidad Total se trabaja con abundancias
...
ANOSIM nos muestra que existen diferencias en las comunidades en sus valores densidad y de
peso que han sido previamente transformadas pero no re escaladas.

Si se consideran las matrices con todas las especies o sin las especies submuestreadas,
los valores muestran patrones semejantes.

Estos patrones son: 1. diferencias significativas en las comunidades considerando la
densidad de los organismos y también considerando los pesos corporales de la fauna
edáfica. 2. Valores de r^2^ interpretados como poder de explicación de la variabilidad de
los datos por parte de los sistemas de uso del suelo bajos, \< a 15 3. Valores de r^2^
siempre más altos para el peso de los organismos que para su densidad.
...
Globalmente se observa separación de los diferentes sitios. El triángulo que es pastizal
naturalizado esta levemente relacionado a los agrícolas.
...
\*\* Análisis de Componentes Prinipales de los sitios de estudio en función de la densidad
de la fauna edáfica \*\*
