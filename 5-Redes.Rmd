...
con la clasificación funcional de las especies tróficas, estos flujos de energía son expresados para cada interacción en Joules/año.
...
para identificar diferencias estadísticamente
...
bm_Lf.r<-c(Lf.r$biomass,
        ...
        1,#2.25*0.0273*1.2*50000*1e6,#Tej_ve
        0.4*0.0273*1.2*50000*1e6)/1e6#humus; 0.4 proxi POM, %MO, 1.2 DAP, 50k volumen suelo
...
F_mc_lu<-data.frame(sitio=c("Reserva","Pastizal","Agrícola"),total=c(sum(flux_R),sum(flux_P),sum(flux_L))/1e3,
           biotransformación=c(F_mo_R/sum(flux_R),F_mo_P/sum(flux_P),F_mo_L/sum(flux_L)
                               )*100,
           detritivoria=c(F_det_R/sum(flux_R),F_det_P/sum(flux_P),F_det_L/sum(flux_L)
                               )*100,
           microfitofagia=c(F_mic_R/sum(flux_R),F_mic_P/sum(flux_P),F_mic_L/sum(flux_L))*100,
           predación=c(F_pred_R/sum(flux_R),F_pred_P/sum(flux_P),F_pred_L/sum(flux_L))*100,
           omnivoría=c(F_omn_R/sum(flux_R),F_omn_P/sum(flux_P),F_omn_L/sum(flux_L))*100,
           row.names = NULL)
...
F_mc_total<-data.frame(sitio=c("Naturalizado","Mixto","Agrícola","Reserva","Pastizal","Agrícola"),total=c(                                                        sum(flux_N),sum(flux_G),sum(flux_A),
                        sum(flux_R.r),sum(flux_P.r),sum(flux_L.r))/1e3,
           biotransformación=c(
                               F_mo_N/sum(flux_N),F_mo_G/sum(flux_G),F_mo_A/sum(flux_A),
                                                             F_mo_R.r/sum(flux_R.r),F_mo_P.r/sum(flux_P.r),F_mo_L.r/sum(flux_L.r))*100,
           detritivoria=c(
                               F_det_N/sum(flux_N),F_det_G/sum(flux_G),F_det_A/sum(flux_A),
                        F_det_R.r/sum(flux_R.r),F_det_P.r/sum(flux_P.r),F_det_L.r/sum(flux_L.r))*100,
           microfitofagia=c(
                               F_mic_N/sum(flux_N),F_mic_G/sum(flux_G),F_mic_A/sum(flux_A),
                               F_mic_R.r/sum(flux_R.r),F_mic_P.r/sum(flux_P.r),F_mic_L.r/sum(flux_L.r))*100,
           predación=c(
                               F_pred_N/sum(flux_N),F_pred_G/sum(flux_G),F_pred_A/sum(flux_A),
                               F_pred_R.r/sum(flux_R.r),F_pred_P.r/sum(flux_P.r),F_pred_L.r/sum(flux_L.r))*100,
           omnivoría=c(
                               F_omn_N/sum(flux_N),F_omn_G/sum(flux_G),F_omn_A/sum(flux_A),
                               F_omn_R.r/sum(flux_R.r),F_omn_P.r/sum(flux_P.r),F_omn_L.r/sum(flux_L.r))*100,
           row.names = NULL)
