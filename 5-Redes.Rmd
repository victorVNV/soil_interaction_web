...
Como valor de estabilidad se utilizará el valor de estabilidad de quasi-signo (\@alesina).
...
Se utilizó el pequete "multiweb" <enlace_github> para calcular las métricas de topología,
las fuerzas de interacciones y la estabilidad de las redes tróficas, con el apoyo de paquetes
...
#Adaptado de COLEMAN y Energetic Fluxweb
...
web0$ef<-NA
web0$ef[web0$taxa%in%mf]<-0.7
web0$ef[web0$taxa%in%micf]<-0.65
web0$ef[web0$taxa%in%pred]<-0.85
web0$ef[web0$taxa%in%det]<-0.35
web0$ef[web0$taxa%in%herb]<-0.125
web0$ef[web0$taxa%in%omn]<-0.4
web0$ef[web0$taxa%in%no_a]<-0.80
...
#Pesos corporales
body_L<-c(L$body,1,1,1,1)
body_R<-c(R$body,1,1,1,1)
body_P<-c(P$body,1,1,1,1)
...
#Pesos corporales
body_N<-c(N$body,1,1,1)
body_G<-c(G$body,1,1,1)
body_A<-c(A$body,1,1,1)
...
webRO$ef[webRO$specie%in%det]<-0.3
webRO$ef[webRO$specie%in%omn]<-0.35
...
#humus, se corrige entre los vectores
...
<!-- However, this level of precision will only very rarely be available, so here we focus on assigning assimilation efficiencies on a more coarse level. It is possible to assign assimilation efficiencies based on a combination of traits, such as resource type and ambient temperature, and for every single species–species interaction in a fully resolved food web. https://doi.org/10.1002/ece3.8060-->
...
F_mic<-sum(flux_R[c("bac","fungi"),colnames(flux_R) %in% omn])
...
2.3e-1, #si considero el promedio entre ambos sería 5.560547e-05, 
                      #pero se coloca -999 cuando no hay info. #nueva info 4/4/24
                      # Andres Pilar et al 2016. soilfoodweb prairie
<!-- # Biomasas
