...
F_mc_lu
```
La detritivoría, la microfitofagia fueron mayores en valor absoluto en la Reserva.
...
La el consumo de la fauna <u>zoofagia</u> se estima como la suma de la 
...
```{r include=FALSE}
Q_med_L<-mean(log(QSS_IS_L$maxre))
Q_med_R<-mean(log(QSS_IS_R$maxre))
Q_med_P<-mean(log(QSS_IS_P$maxre))
```

**Valor de estabilidad de cuasi-signo en suelos de Luján**

En los suelos argiudoles de Luján  
la distribución de la parte real del máximo autovalor (λ) de las matrices de cuasi-signo (Fig. 5.x)
de las redes de interacciones tróficas se movió en un rango de ≈ 7 ordenes de magnitud.
...
par(mfrow=c(1,3))
hist(log(QSS_IS_R$maxre),border =  "blue4",col=NULL,breaks=60,freq = FALSE,main = "Reserva", sub= "Estabilidad de cuasi-signo", xlab = "ln qss",ylab=NULL)
axis(1, at = seq(-6, 3, by = 1))
abline(v=mean(log(QSS_IS_R$maxre)),col="yellow",lty=3,lwd=3)
abline(v=median(log(QSS_IS_R$maxre)),col="red",lty=3,lwd=2.5)
abline(v=c(quantile(log(QSS_IS_R$maxre),probs = 0.25),quantile(log(QSS_IS_R$maxre),probs = 0.75)),col="skyblue",lty=3,lwd=1.5)
hist(log(QSS_IS_P$maxre), border =  "green4",col=NULL,breaks=60,freq = FALSE,main = "Pastizal", sub= "Estabilidad de cuasi-signo", xlab = "ln qss",ylab=NULL)
axis(1, at = seq(-6, 3, by = 1)) 
abline(v=mean(log(QSS_IS_P$maxre)),col="yellow",lty=3,lwd=3)
abline(v=median(log(QSS_IS_P$maxre)),col="red",lty=3,lwd=2.5)
abline(v=c(quantile(log(QSS_IS_P$maxre),probs = 0.25),quantile(log(QSS_IS_P$maxre),probs = 0.75)),col="skyblue",lty=3,lwd=1.5)
hist(log(QSS_IS_L$maxre), border =  "orange3",col=NULL,breaks=60,freq = FALSE,main = "Agrícola", sub= "Estabilidad de cuasi-signo", xlab = "ln qss",ylab=NULL)
axis(1, at = seq(-6, 3, by = 1)) 
abline(v=mean(log(QSS_IS_L$maxre)),col="yellow",lty=3,lwd=3)
abline(v=median(log(QSS_IS_L$maxre)),col="red",lty=3,lwd=2.5)
abline(v=c(quantile(log(QSS_IS_L$maxre),probs = 0.25),quantile(log(QSS_IS_L$maxre),probs = 0.75)),col="skyblue",lty=3,lwd=1.5)

par(mfrow=c(1,1))
#calcular la moda
#mlv(log(QSS_IS_N$maxre), method = "Parzen")
Q_moda_L<-mlv(log(QSS_IS_L$maxre), method = "naive",bw=0.01)
Q_moda_R<-mlv(log(QSS_IS_R$maxre), method = "naive",bw=0.01)
Q_moda_P<-mlv(log(QSS_IS_P$maxre), method = "naive",bw=0.01)
Q_median_R<-median(log(QSS_IS_R$maxre))
```

**Figura 5.x Histogramas del valor de estabilidad. En el eje horizontal, se expresa el valor del máximo autovalor de la matriz Jacobiana obtenidas a partir del algoritmo de quasi-signo (@alesina) expresado en logaritmo natural, se muestran mil iteraciones. Eje vertical valor de frecuencia del intervalo. Como lineas verticales se dibujan, el promedio en color amarillo, la mediana en color rojo, y los cuartiles primero y tercero como lineas más finas.**

Los tres sistemas de uso se diferencian entre sí, en promedio la parte real 
del autovalor de la matriz Jacobiana en los suelos de la comunidad de la Reserva, Pastizal y Agrícola fué
de `r exp(Q_med_R)` - `r exp(Q_med_P)` - `r exp(Q_med_A)`, respectivamente.

La distribución de los valores del λ del QSS se observan (Fig 5.x) con forma similar a los de una campana, pero con excepción de la reserva, el valor más probables, es decir, la moda de la distribución, se halla separada de sus
valores de tendencia centrales. En la Reserva la moda coincide con la mediana siendo esta `r exp(Q_median_R)`, en el pastizal la moda fué `r exp(-0.6)`, en el sistema agrícola la moda fué `r exp(-0.62)`, los valores de estas modas ocurrieron con 56 - 51 - 41 tanto por cien 
en valores de frecuencia respectivamente para cada sistema de uso del suelo. 
