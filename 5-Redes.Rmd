...
los taxones hallados en ambas comunidades de la fauna de suelo
($gamma\; [\gamma]$). A partir de la matriz de interacciones global se
...
particular se corresponde con los taaxones hallados en los diferentes
sistemas de uso del suelo, los valores de pesos corporales, las biomasas
medias y las densidades medias; productos todos desarrollados en las
secciones anteriores (Ver tablas 4.x al x).
...
particulada es el proxi de la cantidad del recurso que se corresponde
...
proporción del contenido en materia orgánica; Digel (.-\@.Digel.2014),
...
recurso efectivamente disponible para la red trófica, entonces, para
...
la tasa metabólica estimada mediante $tasa metabólica = 17.17\; peso^{-0.29}$ y la biomasa de 
los mismos nodos. Con los flujos de energía se pueden caracterizar procesos ecológicos relacionados
con la clasificación funcional de las especies tróficas.

Para realizar comparaciones entre redes tróficas se utilizó la
comparación de distribuciones del método de Anderson-Darling /
Kolmogorov-Smirnoff para identificar diferencias estadísticamente
significativas, mediante el algoritmo de Curveball se establecen redes
...
#Pesos corporales####
body_Lf.r<-c(Lf.r$body,1,1,1,1)/1e6
body_Rf.r<-c(Rf.r$body,1,1,1,1)/1e6
body_Pf.r<-c(Pf.r$body,1,1,1,1)/1e6
...
#plot_troph_level(Gl_net,modules=TRUE,vertexLabel = FALSE, tk = FALSE,sub="Agrícola",edge.color="grey",edge.wigth=0.5)
...
par(mfrow=c(1,3))
plot_troph_level(R_net,modules=TRUE,vertexLabel = FALSE, tk = FALSE,sub="Reserva")
plot_troph_level(P_net,modules=TRUE,vertexLabel = FALSE, tk = FALSE,sub="Pastizal")
plot_troph_level(L_net,modules=TRUE,vertexLabel = FALSE, tk = FALSE,sub="Agrícola")
par(mfrow=c(1,1))
```
...
plot_troph_level(N_net,modules=TRUE,vertexLabel = FALSE, tk = FALSE,sub="Pastizal")
plot_troph_level(G_net,modules=TRUE,vertexLabel = FALSE, tk = FALSE,sub="Mixto")
plot_troph_level(A_net,modules=TRUE,vertexLabel = FALSE, tk = FALSE,sub="Agrícola")
...
En la Reserva la moda de la distribución fué más alta al rededor del primer cuartil lo que implica 
un valor de cuasi-signo cercano a cero con mayor probabilidad de ocurrencia, en el sistema agrícola 
la moda más grande se relaciona con el tercer cuartil de su distribución y en el pastizal la moda está
...
hist(log(QSS_IS_R.r$maxre), main= "Reserva", sub= "logQSS_IS", xlab = NULL)
hist(log(QSS_IS_P.r$maxre), main = "Pastizal", sub= "logQSS_IS", xlab = NULL)
hist(log(QSS_IS_L.r$maxre), main= "Agricola intensivo", sub= "logQSS_IS", xlab = NULL)
...
```{r}
Fplot<-graph_from_adjacency_matrix(flux_R,mode=c("directed") )
```
...
F_herb_R<-sum(flux_R["hummus",])
F_herb_P<-sum(flux_P["hummus",])
F_herb_L<-sum(flux_L["hummus",])

F_herb_N<-sum(flux_N["hummus",])
F_herb_G<-sum(flux_G["hummus",])
F_herb_A<-sum(flux_A["hummus",])


F_herb_R.r<-sum(flux_R.r["hummus",])
F_herb_P.r<-sum(flux_P.r["hummus",])
F_herb_L.r<-sum(flux_L.r["hummus",])


#detritivoros##########
#flujo através de los detritívoros

F_det_R<-sum(flux_R[c("litter","carrion"),!colnames(flux_R) %in% det])
F_det_P<-sum(flux_P[c("litter","carrion"),!colnames(flux_P) %in% det])
F_det_L<-sum(flux_L[c("litter","carrion"),!colnames(flux_L) %in% det])

F_det_N<-sum(flux_N[c("litter","carrion"),!colnames(flux_N) %in% det])
F_det_G<-sum(flux_G[c("litter","carrion"),!colnames(flux_G) %in% det])
F_det_A<-sum(flux_A[c("litter","carrion"),!colnames(flux_A) %in% det])

F_det_R.r<-sum(flux_R.r[c("litter","carrion"),!colnames(flux_R.r) %in% det])
F_det_P.r<-sum(flux_P.r[c("litter","carrion"),!colnames(flux_P.r) %in% det])
F_det_L.r<-sum(flux_L.r[c("litter","carrion"),!colnames(flux_L.r) %in% det])
#microfitfagos###########
#flujo através de los microvíboros

F_mic_R<-sum(flux_R[c("bac","fungi"),colnames(flux_R) %in% micf])
F_mic_P<-sum(flux_P[c("bac","fungi"),colnames(flux_P) %in% micf])
F_mic_L<-sum(flux_L[c("bac","fungi"),colnames(flux_L) %in% micf])

F_mic_N<-sum(flux_N[c("bac","fungi"),colnames(flux_N) %in% micf])
F_mic_G<-sum(flux_G[c("bac","fungi"),colnames(flux_G) %in% micf])
F_mic_A<-sum(flux_A[c("bac","fungi"),colnames(flux_A) %in% micf])
#predadores####
...
F_pred_R.r<-sum(flux_R[,colnames(flux_R.r) %in% pred])
F_pred_P.r<-sum(flux_P[,colnames(flux_P.r) %in% pred])
F_pred_L.r<-sum(flux_L[,colnames(flux_L.r) %in% pred])
...
F_pred_R.r<-sum(flux_R[,colnames(flux_R.r) %in% omn])
F_pred_P.r<-sum(flux_P[,colnames(flux_P.r) %in% omn])
F_pred_L.r<-sum(flux_L[,colnames(flux_L.r) %in% omn])
...
Achiorno, C.L., de Villalobos, C., Ferrari, L., 2008. Toxicity of the
herbicide glyphosate to Chordodes nobilii (Gordiida, Nematomorpha).
Chemosphere 71 (10): 1816– 1822. Los nematomorfas adultos son de via
libre, los parásitos son sus larvas

El autor trabaja sobre la abundancia de individuos del suelo a escala
global, y encontró patrones, entre la temp, la masa, y al transferencia
a traves de niveles troficos siendo estos similares a los que se hallan
o se sabían eran patrones generales en los ecosistemas For example,
though abundance and body masses of litter and soil animals can be
estimated fairly simply (Coleman et al. 1999), it can be extremely
difficult to accurately estimate resource availability and average body
temperature in the soil environment. Meanwhile, energetic resources
available to soil invertebrates at a given locale can vary at the scale
of centimeters (Coleman et al. 2004). important physical and biological
factors. These factors could include physical conditions of the litter
and soil environment, such as pH, nutrient content, and moisture
availability, which are known to affect soil invertebrate community
structure (Coleman et al. 2004, Reich et al. 2005). Also, the
stoichiometric profiles of resources relative to consumers are known to
affect rates of resource consumption ENERGY USE AND ANIMAL ABUNDANCE IN
LITTER AND SOIL COMMUNITIES TIMOTHY D. MEEHAN 2006
