...
plot_troph_level(R_net,modules=FALSE,vertexLabel = FALSE, tk = FALSE,main="Reserva")
plot_troph_level(P_net,modules=FALSE,vertexLabel = FALSE, tk = FALSE,main="Pastizal")
plot_troph_level(L_net,modules=FALSE,vertexLabel = FALSE, tk = FALSE,main="Agrícola")
...
plot_troph_level(N_net,modules=FALSE,vertexLabel = FALSE, tk = FALSE,main="Pastizal")
plot_troph_level(G_net,modules=FALSE,vertexLabel = FALSE, tk = FALSE,main="Mixto")
plot_troph_level(A_net,modules=FALSE,vertexLabel = FALSE, tk = FALSE,main="Agrícola")
...
F_mc_lu<-data.frame(sitio=c("Reserva","Pastizal","Agrícola"),
                    total=c(paste(round(sum(flux_R)/1e6,1),"a"),paste(round(sum(flux_P)/1e6,1),"a"),
                            paste(round(sum(flux_L)/1e6,1),"b")
                               ),
           ...
           herbivoría=c(F_herb_R,F_herb_P,F_herb_L),
           row.names = NULL)


```

**Tabla 5.x Flujos de energía en Joule/año (flujo total en MJ/año) que atraviesan los diferentes canales de energía vinculados a funciones biológicas que ocurren en las redes tróficas de la comunidades de suelos Argiudoles de Luján**
...
fauna del que se alimentan los omnívoros, la contribución de los omnívoros a otros canales de energía como
por ejemplo el canal de la microfitofagía se halla contabilizada en el canal en cuestión.
La zoofagia, en términos absolutos, fué mayor en el pastizal (≈12 kJ.año^-1^) y 
menor en el sistema agrícola(≈7 kJ.año^-1^), con relación al total la importancia de la zoofagia fué cercana entre el pastizal y el sistema agrícola al rededor del 0,58% y menor en la Reserva con una importancia del 0,37%.

La zoofagia por parte de los omnivoros es el tercer canal de energía en importancia a través de estos actores 
llega hasta el ≈ 30% del flujo energético anual, en el pastizal su importancia fue ligeramente mayor.
La cantidad de energía que llega a los depredadores es el de menor importancia relativa por debajo del 1% del 
flujo total, en los suelos de la reserva la cantidad de energía en este canal fué mas bajo, seguido por el 
sistema agrícola y el pastizal.
...
F_mc_total<-data.frame(sitio=c("Naturalizado","Mixto","Agrícola","Reserva","Pastizal","Agrícola"),
           biotransformación=c(paste(sep="",round((F_mo_N/1000),2)," ","(",round((F_mo_N/sum(flux_N))*100,2),")"),
                               paste(sep="",round((F_mo_G/1000),2)," ","(",round((F_mo_G/sum(flux_G))*100,2),")"),
                               paste(sep="",round((F_mo_A/1000),2)," ","(",round((F_mo_A/sum(flux_A))*100,2),")"),
                               paste(sep="",round((F_mo_R.r/1000),2)," ","(",round((F_mo_R.r/sum(flux_R.r))*100,2),")"),
                               paste(sep="",round((F_mo_P.r/1000),2)," ","(",round((F_mo_P.r/sum(flux_P.r))*100,2),")"),
                               paste(sep="",round((F_mo_L.r/1000),2)," ","(",round((F_mo_L.r/sum(flux_L.r))*100,2),")")
                               ),
           detritivoria=c(paste(sep="",round((F_det_N/1000),2)," ","(",round((F_det_N/sum(flux_N))*100,2),")"),
                               paste(sep="",round((F_det_G/1000),2)," ","(",round((F_det_G/sum(flux_G))*100,2),")"),
                               paste(sep="",round((F_det_A/1000),2)," ","(",round((F_det_A/sum(flux_A))*100,2),")"),
                               paste(sep="",round((F_det_R.r/1000),2)," ","(",round((F_det_R.r/sum(flux_R.r))*100,2),")"),
                               paste(sep="",round((F_det_P.r/1000),2)," ","(",round((F_det_P.r/sum(flux_P.r))*100,2),")"),
                               paste(sep="",round((F_det_L.r/1000),2)," ","(",round((F_det_L.r/sum(flux_L.r))*100,2),")")
                               )
            ,
           microfitofagia=c(paste(sep="",round((F_mic_N/1000),2)," ","(",round((F_mic_N/sum(flux_N))*100,2),")"),
                               paste(sep="",round((F_mic_G/1000),2)," ","(",round((F_mic_G/sum(flux_G))*100,2),")"),
                               paste(sep="",round((F_mic_A/1000),2)," ","(",round((F_mic_A/sum(flux_A))*100,2),")"),
                               paste(sep="",round((F_mic_R.r/1000),2)," ","(",round((F_mic_R.r/sum(flux_R.r))*100,2),")"),
                               paste(sep="",round((F_mic_P.r/1000),2)," ","(",round((F_mic_P.r/sum(flux_P.r))*100,2),")"),
                               paste(sep="",round((F_mic_L.r/1000),2)," ","(",round((F_mic_L.r/sum(flux_L.r))*100,2),")")
                               ),
           predación=c(paste(sep="",round((F_pred_N/1000),2)," ","(",round((F_pred_N/sum(flux_N))*100,2),")"),
                               paste(sep="",round((F_pred_G/1000),2)," ","(",round((F_pred_G/sum(flux_G))*100,2),")"),
                               paste(sep="",round((F_pred_A/1000),2)," ","(",round((F_pred_A/sum(flux_A))*100,2),")"),
                               paste(sep="",round((F_pred_R.r/1000),2)," ","(",round((F_pred_R.r/sum(flux_R.r))*100,2),")"),
                               paste(sep="",round((F_pred_P.r/1000),2)," ","(",round((F_pred_P.r/sum(flux_P.r))*100,2),")"),
                               paste(sep="",round((F_pred_L.r/1000),2)," ","(",round((F_pred_L.r/sum(flux_L.r))*100,2),")")
                               ),
           omnivoría=c(paste(sep="",round((F_omn_N/1000),2)," ","(",round((F_omn_N/sum(flux_N))*100,2),")"),
                               paste(sep="",round((F_omn_G/1000),2)," ","(",round((F_omn_G/sum(flux_G))*100,2),")"),
                               paste(sep="",round((F_omn_A/1000),2)," ","(",round((F_omn_A/sum(flux_A))*100,2),")"),
                               paste(sep="",round((F_omn_R.r/1000),2)," ","(",round((F_omn_R.r/sum(flux_R.r))*100,2),")"),
                               paste(sep="",round((F_omn_P.r/1000),2)," ","(",round((F_omn_P.r/sum(flux_P.r))*100,2),")"),
                               paste(sep="",round((F_omn_L.r/1000),2)," ","(",round((F_omn_L.r/sum(flux_L.r))*100,2),")")
                               ),
             row.names = NULL)
...
**Tabla 5.x Flujos de energía en Joule/año y en entre paréntesis el porcentaje con respecto al flujo total que atraviesan los diferentes canales de energía vinculados a funciones biológicas que ocurren en las redes tróficas de las comunidades de microartrópodos edáficos y lombrices de tierra de suelos Argiudoles de Chivilcoy y Navarro y suelos argiudoles de Luján**
...
En los suelos de Chivilcoy y Navarro, (Tabla 5.x) los flujos de energía alcanzan un máximo de 81,2
kJ.año^-1^ en los suelos con actividad agrícola ganadera, seguidos por el pastizal naturalizado con 78,7 
kJ.año^-1^ y el sistema agrícola intensivo que suma 78,1 kJ.año^-1^. 

En la red trofica con actores similares a los que ocurren en Chivilcoy y Navarro pero en los suelos 
argiudoles de Luján en promedio el flujo total de energía fué de 1,65 MJ.año^-1^, es decir, que 
através de la red trofica de Luján se estima un flujo de energía 21 veces mayor.

El canal que mueve el humus del suelo fue el de mayor importancia, superando el 95% de la 
contribución al flujo 
total de energía en suelos argiudoles de Chivilcoy y Navarro, este fué mayor en términos relativos
en el sistema del pastizal naturalizado y menor en el sistema mixto agrícola ganadero; en suelos de Luján
tuvo importancias similares en la Reserva y el Pastizal y menor importancia en el 
sistema Agrícola, aquí también la contribución de este canal fué mayor al 95%. 

La detritivoría (Tabla 5.x) fue de poca importancia en el pastizal naturalizado a la vez que fué mayor en el sistema mixto agrícola ganadero; por su parte en suelos de Luján en el sistema de la reserva se
observó la mayor participación de detritívoros con relación al total. La microfitofagia en el pastizal naturalizado fué de menor importancia separandose casi dos puntos porcentuales de aquel que ocurre en el sistema mixto; en suelos de Luján esta fue mayor en el sistema Agrícola y menor en la Reserva. 

La zoofagia, en suelos de Chivilcoy y Navarro, contabiliza la mayor contribución en los suelos con actividad agrícola ganadera en este la importancia del flujo a traveś de los predadores verdaderos fué similar a la que ocurre en el pastizal naturalizado pero es la zoofagia de los omnívoros la que contribuyen con un mayor flujo de energía. La predación fué mayor en suelos con sistema agrícola. En suelos de Luján la predación en el pastizal y en el sistema agrícola fué mayor, la zoofagia por omnívoros fue mayor en la reserva y en el sistema agrícola, en total, la zoofagia fue mayor en el pastizal.
...
En los suelos argiudoles de Luján  
...
que resultan de las iteraciones hechas sobre las matrices Jacobianas de las redes tróficas de detritívoros 
edáficos que ocurren en los diferentes sistemas de uso del suelo, se hallaron diferencias significativas entre 
...
Los tres sistemas de uso se diferencian entre sí, el promedio y la mediana de la parte real 
...
La distribución de los valores λ del QSS se asemejan (Fig 5.x) a los de una campana simétrica, el valor más probable, es decir, la moda de la distribución, se halla cercana a los valores de tendencia centrales en los diferentes sistemas de uso del suelo. En la Reserva la moda la moda fué `r exp(Q_moda_R)`, en el pastizal la moda fué `r exp(Q_moda_P)`, en el sistema agrícola la moda fué `r exp(Q_moda_L)`, los valores de estas modas ocurrieron con una frecuencia mayor a 40 tanto por cien 
en los diferentes sistemas de uso del suelo. 

Estos datos nos muestran que en sistemas antropizados se obtienen valores del λ del cuasi-signo más cercanos a cero.
...
1931), esta red trófica edáfica podria estructurarse como los recursos bases que son consumidos 
...
explica el valor del nivel trófico medio, alrededor de tres.
...
El coeficiente de agupamiento "clustering", que implica la probabilidad de que diferentes especies tróficas 
esten cercanamente interconectadas debido al solapamiento de predadores o de sus dietas, fué 
