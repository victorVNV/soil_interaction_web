...
interacciones [@sole2001], las redes de interacciones nos
muestran todas las posibles tramas en las que las especies pueden
interactuar [@delmas2019; @kefi2012] esta pueden
...
mutualistas, entre otras [@sole2001; @labrador2008].

<!-- Importancia de redes en general, se puede integrar con el parrafo anterior -->
El enfoque que permite integrar la diversidad de especies, las interacciones interespecíficas, las interacciones con el ambiente, la redundancia funcional, el papel de las especies clave y la multiplicidad de canales de materia y energía que ocurren en los ecosistemas es el estudio de la red de interacciones [@potapov2022;@barrios2007;@briones2014]. Este enfoque simplifica la complejidad de las interacciones y conecta con diferentes parámetros emergentes del estudio de los ecosistemas [@hooper2005; @delmas2019].
...
los canales por los cuales se mueve la materia y la energía [@barnes2018],
...
de especies con un papel semejante en el funcionamiento de un ecosistema [@gitay1997]
que permiten el mantenimiento de las funciones
ecológicas del suelo [@quevreux2023].
...
antrópicas [@kardol2016] que ocurren debidas al manejo del suelo con fines agropecuarios,
...
omnívoros, la vulnerabilidad, la generalidad, etc. [@digel2014; @delmas2019; @maggiotto2019].
Por lo tanto se obtienen patrones generales sobre
la topología de la red, sobre las consecuencias en la 
dinámica de la comunidad y las especies constituyentes [@kefi2012; @pimm1991]
y sobre los efectos en las
funciones de los ecosistemas [@sole2001; @labrador2008].
...
y por lo tanto en el estado de conservación del mismo que
...
tanto la mayoría de las especies poseen pocas conexiones [@delmas2019; @pimm1991; @sole2001] 
y este es el modo en que la
diversidad biológica y el funcionamiento de los ecosistemas están
ligados [@balvanera2006]. En el ecosistema del suelo priman
los grupos funcionales con especies redundantes [@martin-lopez2007]
estos grupos funcionales permiten una mayor capacidad de respuesta ante las perturbaciones 
(**resiliencia**) asegurando el mantenimiento de las funciones en el ecosistema[@delmas2019; @pimm1991; @sole2001],
manteniendo la estructura, el funcionamiento y los mecanismos de
autorregulación del ecosistema [@martin-lopez2007] aunque la desaparición
de especies muy conectadas puede desencadenar extinciones secundarias y
fragmentaciones de la red [@sole2001].

<!-- Repite ya dijimos que es util --> 
Los estudios de redes de interacciones proporcionan información acerca
de la estructura autoorganizada de los ecosistemas [@oneill1986].
...
las perturbaciones debidas al manejo del suelo [@delmas2019; @maggiotto2019]. En las
...
el ciclo de nutrientes, el control de poblaciones, etc. 
mediante el estudio de los flujos de energía a través de diferentes 
canales de energía y las posibles diferencias en
las redes tróficas de las comunidades de diferentes sistemas de uso del
suelo [@scheu2002; @potapov2023].
...
Digel et al. [-@digel2014] utilizó las redes tróficas de 48 comunidades del
...
Maggioto et al. [-@maggiotto2019] utilizaron las herramientas del análisis de las
...
para poder modelar la matriz de interacciones tróficas [@delmas2019], 
el peso  corporal y la densidad de los taxas involucrados [@laigle2018]
 que permite conocer las fuerzas de
interacciones [@deruiter1995], los flujos de energía através de
la red [@potapov2022; @barnes2018] y calcular la resiliencia
y estabilidad de los sistemas bajo estudio [@moore2012; @gauzens2019; @allesina2008].
...
<!--Roughly speak-ing, complexity may be measured by the number andnature of the individual links in the trophic web,and stability by the tendency for relatively smallpopulation perturbations to damp out, returning thesystem to its equilibrium configuration. QUALITATIVE STABILITY IN MODEL ECOSYSTEMS1 RoBERT M. MAY -->
...
La matriz de interacciones global con sus valores de preferencias alimenticias puede consultarse en el
(Apéndice 5.1).
...
medias y las densidades medias (Ver tablas del capítulo cuatro 4.1 al 4.5).
...
Para el recurso trófico "humus" se considera que el 50% de la materia orgánica
...
por un lado el criterio de Digel [-@digel2014],
...
como sugiere Pilar et al. [-@andres2016] y que resulta en un valor de
$2,3 \,x\,7\,^{-10} g\,m^{-2}$.
...
Thakur comprender Towards an integrative understanding of soil biodiversity quien explica hotspot y micrositios, entonces no todos llegan a bacteris por restriccion de microporos, Coleman en fundamental muestra las escalas Soil Food Webs:
Detritivory and
Microbivory in Soils y una imagen para justificar esto. Basicamente decir que no llegan los animales a capturar a odos los ongos y bacterias--> entonces, para
...
trófico, respectivamente. <!-- que aportan las metricas en relacion al nuestra hipotesis, 
...
y los coeficientes de asimilación [@coleman2004; @moore2012], para la microflora, 
...
interacción en Joules/año [@gauzens2019;@moore2012]. <!-- falta la explicacion de la clasificación funcional-->
...
biomasa de las presas/recursos y lo expresa [@pawar2009].
...
nodos [@delmas2019]. Para cada aleatorizacion producto del algoritmo se calcula el  el valor del máximo autovalor,
...
**Figura 5.1** Representación de las redes tróficas de la comunidad detritívora de la fauna edáfica de suelos argiudoles típicos de Luján para cada sistema de uso del suelo. El tamaño de los nodos representa al grado del nodo, los colores forman un gradiente desde el verde hacia el rojo para indicar especies base hacia especies tope respectivamente. La distribución de los nodos a lo largo del eje horizontal sigue un patron semi-aleatorio ("jitter") por lo tanto no se vincula a caracteristicas del nodo en particular. Sobre el eje vertical se indica el nivel trófico.
...
**Tabla 5.1.** Tabla de índices topológicos de las redes tróficas de los diferentes sistemas de uso de los suelos Argiudoles típicos de la Pampa Ondulada. 
...
La cantidad de energía ligada a la <u>biotransformación</u> del suelo (Table 5.2),
...
edáficos, la mayor participación de la microflora se observó en el sistema Agrícola (Tabla 5.2).
...
En términos relativos la detritivoría fué mayor en el sistema agrícola (≈ 30%) y fué menor en el pastizal (≈ 20%). A la vez (Tabla 5.2) 
...
**Tabla 5.3** Flujos de energía en Joule/año, en entre paréntesis el porcentaje del flujo de energía en relación al flujo total y entre corchetes el porcentaje del flujo de energía que llega a la microflora. Las columnas corresponden a los diferentes canales de energía vinculados a las funciones biológicas que ocurren en las redes tróficas de las comunidades de microartrópodos edáficos y lombrices de tierra de suelos Argiudoles de Chivilcoy y Navarro y suelos argiudoles de Luján
...
En los suelos de Chivilcoy y Navarro, (Tabla 5.3) los flujos de energía alcanzan un máximo de 37,74
...
La detritivoría (Tabla 5.3) fue menor en el pastizal naturalizado con una gran participación de la microflora en el consumo del recurso 
...
de las redes de interacciones tróficas (Fig. 5.3) se movió en un rango de ≈ 7 ordenes de magnitud.
...
Estas redes tróficas tienen 4 niveles tróficos como los encontrados en redes tróficas de otros sistemas ecológicos 
[@pimm1991; @quevreux2023],
esta red trófica edáfica podria estructurarse como los recursos bases que son consumidos 
...
se estructuran como redes reticulares [@wolkovich2016] que implica que los animales de mayor porte utilizan 
recursos en niveles tróficos similares a aquellos pertenecientes a la red trófica "micro-" (*"microfood web"* en 
inglés), por ejemplo que las lombrices de tierra
cuyo espectro de tamaño es alto se alimente de hojarasca [@quevreux2023;@potapov2021a].
...
moderado en las redes tróficas de suelo argiudoles típicos (C.C. > 0,4), según Digel et al  [-@digel2014]
en los suelos 
las redes tróficas de detritívoros muestran un valor del coeficiente de agrupamiento más alto que el de 
las demás redes ecológicas abordadas por estos autores (ecosistemas terrestres basado en plantas y 
ecosistemas de acuaticos) 
aunque el valor calculado para los suelos locales de la Pampa Ondulada fueron de más del doble que 
...
En general los valores de los parámetros topológicos entre los diferentes sistemas de uso del suelo fueron próximos 
entre sí,
...
enquitreidos 
que son poco estudiados en los suelos argentinos. Si existen configuraciones que varían entre los sistemas de uso 
del suelo debidas la riqueza de especies tróficas como al número de interacciones con el que participan lo que 
determina la variabilidad entre parámetros.
