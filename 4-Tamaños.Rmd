...
$$B_i = \mu + \text{S}_i + \text{G}_j + \text{S x G}_{ij} + \epsilon_i$$
...
en los valores de biomasa ($q: p_{valor}<0.05$) entre el sistema mixto agrícola ganadero con un mayor valor de biomasa en promedio y el sistema del pastizal naturalizado, el sistema agrícola intensivo se encuentra en una situaciń intermedia.
...
#El objetivo es ver el efecto e los factores sobre las pendientes.
#modelo UNLu
rqU<-rq(log(D) ~ (sitio + gremio + log(bio.ind))^2 ,tau = .75, data = baseU)
#modelo Ro
rqRO<-rq(log(abundance) ~ (system+guild+log(bio.ind))^2, tau = .75, data = base)

#esto permite ver las interacciones entre todos los grupos UNLu
baseU %>% mutate(sitio = factor(sitio,levels=c("R","P","A")),
                gremio = factor(gremio,levels=c("herbivoro","predador","microfitofago",
                                       "detritivoro","omnivoro")))%>%
  rq(log(D) ~ sitio + gremio + log(bio.ind) + sitio:log(bio.ind) + 
    gremio:log(bio.ind), tau = .75, data = .)%>%
  summary(.,se="boot")

#esto permite ver las interacciones entre todos los grupos RO
base %>% mutate(system = factor(system,levels=c("N","G","A")),
                guild = factor(guild,levels=c("predator","microphytophages",
                                              "detritivore","omnivore")))%>%
  rq(log(abundance) ~ (system+guild+log(bio.ind))^2, tau = .75, data = .)%>%
  summary(.,se="boot")
...
for (i in seq_along(niveles_sitio)) {
    for (j in seq_along(niveles_gremio)) {
        ...
        resultado <- baseU %>% 
            ...
            rq(log(D) ~ (sitio + gremio + log(bio.ind))^2,
               tau = .75, data = .) %>%
            summary(., se = "boot")
...
```{r qqplotU+h,include=FALSE,warning=FALSE,message=FALSE}
...
```{r qqplotU,include=FALSE,warning=FALSE,message=FALSE}
...
base%>%ggplot(aes(x=log(bio.ind), y=log(abundance),color=guild))+geom_point(size=1.1,shape=8,color=1)+geom_quantile(quantiles = 0.75, size=1.5) +  scale_color_viridis_d()+facet_grid(system~.)+  labs( x=expression(paste("Peso corporal","  ",paste(ln," ",mu,"g")[ps])), y=expression(paste("Densidad","  ",ln," ","n°"[ind]/m^{2})))
