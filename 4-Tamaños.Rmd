...
En los suelos de Luján las diferencias en la biomasa media con respecto al intercepto 
se explican como diferencias que ocurren en las biomasas medias entre los 
diferentes gremios tróficos, no se explican diferencias en las biomasas medias por
posibles diferencias relacionadas a los diferentes sistemas de uso del suelo.
...
Los gremios tróficos fueron diferentes significativamente entre sí ($q :p_{valor}<0.05$) con excepción de los gremios de omnívoros y microfitófagos que comparten valores de biomasas medias similares.
Los detritívoros son el gremio con la mayor biomasa media, luego los microfitófagos y omnívoros, siguen a estos los predadores y por último los herbívoros.
...
Al nivel de las interacciones, no se observaron diferencias significativas <u>dentro</u> de los gremios tróficos comparados entre los diferentes sistemas de uso del suelo, si existen diferencias significativas ($q :p_{valor}<0.05$) <u>entre</u> diferentes gremios tróficos y diferentes sistemas de uso del suelo.

Las biomasas de los gremios tróficos son similares entre los detritívoros y omnívoros y entre los predadores y los microfitófagos. Los detritívoros y omnívoros expresan la mayor biomasa media, los microfitófagos y predadores siguen en órden de biomasas respectivamente.
...
Entre los sistemas de uso del suelo, existen diferencias significativas en los valores de biomasa media ($q: p_{valor}<0.05$) entre el sistema mixto agrícola ganadero con el mayor valor de biomasa total en promedio y el sistema del pastizal naturalizado con el menor valor de biomasa total en promedio, el sistema agrícola intensivo muestra biomasa media total en una situación intermedia.

## Relación entre el espectro de pesos corporales promedio y la densidad de los taxones.
...
-->
```
El modelo de regresión de cuartiles busca la relacion entre el peso corporal promedio de los taxones de cada sitio con la densidad de los mismos y la relación con los efectos del gremio trófico y los sistemas de uso del suelo en cada localidad. 

En los suelos argiudoles de Luján el modelo indica que existen efectos de los gremios tróficos sobre la variación de las pendientes. Los sistemas de uso del suelo no tuvieron efecto sobre cambios en la pendiente.

$$ D_{ijk}^{\;\;\tau} = \beta_0^{\;\;\tau} + \beta_i^{\;\;\tau} + \beta_iG_j^{\;\;\tau} + S_k^{\;\;\tau} + G_j^{\;\;\tau} + (SxG)_{jk}^{\;\;\tau} + \epsilon^{\;\;\tau} $$

La interacción entre el sistema de uso del suelo y los gremios tróficos marcan un efecto sobre el intercepto del modelo.
...
En la figura 4.x se observa los diferentes gremios tróficos con las regresiones sobre cada sistema de uso del suelo. Se observan diferentes rangos de distribución de los pesos corporales promedios, los omnívoros son la categoría trófica con el menor rango de distribución en tamaños. Los taxones de mayor peso ocurrieron en la categoría de los detritívoros en el que se encuentran las lombrices de tierra y los coleópteros scarabeidos, las categorías tróficas con los taxones de menor peso son aquellos microfitófagos y herbívoros que incluyen a los nemátodos.

Al nivel de las interacciones el intercepto de los predadores de los sistemas R y P en comparación con los microfitófagos del sistema A difirió significativamente ($>|\;t\;| : p_{value}<0.05$¹) además el intercepto de los predadores del sistema A difirió de los microfitófagos de R y P, entre ambos gremios tróficos y en cada sitio, los interceptos difieren en 1,1 ordenes de magnitud aproximadamente .

Entre gremios tróficos los interceptos fueron diferentes para detritívoros en comparación con los predadores ($dif:-1.41768$) y omnívoros ($dif:-2.18830$); para los microfitófagos hubo diferencias también con los omnívoros ($dif: -1.80813$) y con los predadores ($dif: -1.03751$), los hervívoros no tuvieron diferencias con los demás gremios en términos del intercepto.

Las pendientes de los gremios tróficos en general tienen valores negativos, correspondiéndose con el hecho de que los organismos más abundantes son aquellos de menor peso promedio por taxón, con excepción de la categoría del gremio de los omnívoros, cuyos valores de pendientes no fueron significativamente diferentes de cero ($>|\;t\;| : p_{value}<0.05$) esto implica que los omnívoros con diferentes tamaños corporales tienen abunancias similares en estos suelos, aunque la extensión en peso corporal se encuentra por debajo de $e^5 \approx 150 \mu g$. Además, la pendiente de los herbívoros difirió de la pendiente de todos los gremios tróficos ($>|\;t\;| : p_{value}<0.1$) en general siendo la pendiente de los herbívoros diferente de las demás en $dif: \;\;\approx 0,5$.

Se observa que tanto los nemátodos omnívoros como los microartrópodos omnívoros tienen relaciones de abundancias y pesos corporales solapados con los nemátodos siendo estos los más abundantes. El gremio de los detritívoros no tiene taxones de nemátodos en su categoría debido a los hábitos alimenticios de estos y a sus estructuras bucales, se observa que las abundancias de la mesofauna y de la macrofauna se separan indicando mayores abundancias para la mesofauna, aunque en términos de pesos corporales promedios se observan pesos superpuestos entre la mesofauna más pesada y la macrofauna más pequeña. Los predadores de la fauna del suelo tienen un rango de tamaños similar al de los microfitófagos, se observaron entre los predadores de menor tamaño animales de la mesofauna con pesos corporales menores al de los nemátodos (microfauna), esto puede ser debido a formas juveniles de la mesofauna predadora o a que los nemátodos predadores son animales "pesados".
