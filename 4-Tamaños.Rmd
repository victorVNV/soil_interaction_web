...
emms_U <- emmeans::emmeans(rqU, ~ sitio:gremio)
emms_RO <- emmeans::emmeans(rqRO, ~ system:log(bio.ind)+guild:log(bio.ind))

# Realizar las comparaciones por pares
pairwise_comparisons_U <- contrast(emms_U, method = "pairwise", adjust = "bonferroni")
pairwise_comparisons_RO <- contrast(emms_RO, method = "pairwise", adjust = "bonferroni")

# Mostrar los resultados
summary(pairwise_comparisons_U)
summary(pairwise_comparisons_RO)
...
En los argiudoles de Chivilcoy y Navarro, la regresión de quantiles evaluada entre la densidad y el peso promedio de los taxones de microartrópodos edáficos muestra un efecto de los sistemas de uso del suelo sobre los cambios en las pendientes, también un efecto del gremio trófico sobre las pendientes y el efecto de la interacción entre el sistema de uso y los gremios tróficos sobre el intercepto.

$$ D_{ijk}^{\;\;\tau} = \beta_0^{\;\;\tau} + \beta_i^{\;\;\tau} + \beta_iG_j^{\;\;\tau} + \beta_iS_k^{\;\;\tau} + S_k^{\;\;\tau} + G_j^{\;\;\tau} + (SxG)_{jk}^{\;\;\tau} + \epsilon^{\;\;\tau} $$


Las pendientes de los gremios tróficos de microartrópodos edáficos muestran valores negativos con excepción del gremio de los omnívoros (Fig. 4.x), esto implica que los omnivoros más pesados son más abundantes, hay que considerar también que el rango de distribución de pesos promedios de los taxones de microartrópodos omnívoros es también acotado, estos artrópodos tienen pesos inferiores a $e^3 \approx 20 \mu g$

Los microartrópodos microfitófagos exiben el mayor rango de tamaños corporales, aproximadamente 6 órdenes de magnitud, y en esta categoría se observaron en el sistema del pastizal naturalizado los taxones con el menor peso corporal promedio. Los detritívoros y predadores tienen un rango de pesos corporales similares, pero los predadores son una categoría poco abundante.

En general para cada gremio trófico las lineas de regresión sobre el tercer cuartil que representan al pastizal naturalizado se observaron por debajo de las demás lineas de regresión aunque esto se revierte para el caso de los microartrópodos predadores del pastizal naturalizado indicando que existen más predadores entre los diferentes pesos corporales.

Al nivel de las interacciones entre los gremios y sistemas de uso del suelo se reportan diferencias significativas ($>|\;t\;| : p_{value}<0.1$) en los interceptos entre: a) los detritívoros del sistema naturalizado en relación a los predadores del sistema mixto y agrícola y diferencias con los microfitófagos del sistema agrícola; b) los microfitófagos en el pastizal naturalizado y los omnivoros del sistema agrícola y los detritívoros del sistema mixto; c) los omnívoros del pastizal naturalizado en comparación con los predadores y microfitófagos del sistema A; d) los predadores del pastizal naturalizado difieren de los detritívoros del sistema agrícola intensivo y agrícola mixto y también de los omnívoros de A; además e) los detritívoros, microfitófagos y predadores del sistema agrícola difirieron de los omnívoros en el sistema mixto; f) los omnívoros del sistema agrícola intensivo se diferenciaron de los predadores, detritívoros y microfitófagos edel sistema mixto agrícola ganadero.
...
```
Las diferencias en las interacciones observadas sobre el intercepto indican diferencias en los valores de las densidades de los diferentes gremios tróficos dentro de cada sistema de uso, los interceptos de los gremios tróficos sin considerar sus interacciones en cada sistema de uso indican que los predadores y omnívoros comparten interceptos similares y que tanto los detritívoros como los microfitófagos se diferencian de los demás gremios tróficos.

El efecto de los diferentes gremios sobre la pendiente de regresión de cuantiles muestra que los omnívoros siempre fuéron diferentes significativamente, no hallándose otras diferencias significativas en los demás gremios tróficos, esto puede deberse a que la pendientes positivas de los omnívoros en cada sistema de uso enmascara las diferencias entre los demás gremios. En cambio, el efecto de los sistemas de uso del suelo sobre la pendiente mostró diferencias significativas entre el sistema del pastizal naturalizado y el sistema agrícola intensivo siendo la pendiente en este sistema mayor ($dif: 0,23$), esta relación se establece a través de cada gremio trófico inclusive a aquellos con pendientes positivas. Una pendiente más pronunciada, es decir valores de pendientes más alejados de cero, indica que organismos más pesados, mas grandes o de mayor tamaño, se encuentran en baja abundancia o no se encontraron en los sitios estudiados.

La regresión sobre los sistemas de uso del suelo sin considerar los gremios tróficos muestran valores del interceptos diferentes entre entre sí ($8.06 \;(A) > 8.02 \;(G) > 7.98 \;(N)$), además, con respecto a la pendiente, el valor para el sistema mixto no mostró ser diferente de cero.
...
emms_RO3 <- emmeans::emmeans(rqRO, ~ system + guild:log(bio.ind))
pairwise_comparisons_RO3 <- contrast(emms_RO3, method = "pairwise", adjust = "BH")
